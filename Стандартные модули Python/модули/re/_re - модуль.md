...
|`(?P<name>...)`|Именованная группа с захватом. Одновременно с порядковым номера присваивается имя группы|`<(?P<tag>[hH][1-6])>\w+<(?P=tag)>`|<mark><u>&lt;H1&gt;Заголовок&lt;/H1&gt;</u></mark><br>&lt;h2&gt;Заголовок&lt;/h3&gt;|
...
|`\A`|Аналог `^` не учитывающий настройку `MULTILINE`|||
...
|`\Z`|Аналог `$` не учитывающий настройку `MULTILINE`|||
