...
|**[`disk_usage()`](https://docs-python.ru/standart-library/modul-shutil-python/funktsija-disk-usage-modulja-shutil/ "Функция disk_usage() модуля shutil в Python.")**|возвращает статистику использования диска по указанному пути в виде именованного [кортежа](https://docs-python.ru/tutorial/osnovnye-vstroennye-tipy-python/tip-dannyh-tuple-kortezh/ "Кортеж tuple в Python.") с атрибутами `total`, `used` и `free`.|
|**[`chown()`](https://docs-python.ru/standart-library/modul-shutil-python/funktsija-chown-modulja-shutil/ "Функция chown() модуля shutil в Python.")**|меняет владельца пользователя и/или группы по указанному пути.|
|**[`which()`](https://docs-python.ru/standart-library/modul-shutil-python/funktsija-which-modulja-shutil/ "Функция which() модуля shutil в Python.")**|возвращает путь к исполняемому файлу, который будет запущен, если будет вызван данный `cmd`. Если никакая команда не будет вызвана, вернет `None`.|
|**[`make_archive()`](https://docs-python.ru/standart-library/modul-shutil-python/funktsija-make-archive-modulja-shutil/ "Функция make_archive() модуля shutil в Python.")**|создает архивный файл, например `zip` или `tar` и возвращает его имя.|
|**[`get_archive_formats()`](https://docs-python.ru/standart-library/modul-shutil-python/funktsija-get-archive-formats-modulja-shutil/ "Функция get_archive_formats() модуля shutil в Python.")**|возвращает список поддерживаемых форматов для архивирования.|
|**[`register_archive_format()`](https://docs-python.ru/standart-library/modul-shutil-python/funktsija-register-archive-format-modulja-shutil/ "Функция register_archive_format() модуля shutil в Python.")**|регистрирует архиватор для имени формата `name` в списке поддерживаемых форматов|
|**[`register_unpack_format()`](https://docs-python.ru/standart-library/modul-shutil-python/funktsija-register-unpack-format-modulja-shutil/ "Функция register_unpack_format() модуля shutil в Python.")**|регистрирует формат для распаковки архива.|
