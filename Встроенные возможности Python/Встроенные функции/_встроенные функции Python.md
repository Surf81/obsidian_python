...
#### Возвращаемое значение:
-   файловый объект (поток).
объект, предоставляющий файлово-ориентированный API с такими методами, как `file.read()` или `file.write()` базовому ресурсу
...
```python
fp = open("foo.txt", "r+")
fp.name              # /home/python-3.8/foo.txt
```
...
Не может применяться к множествам `set()`
