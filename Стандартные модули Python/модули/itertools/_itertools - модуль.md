...
- **chain**
- **compress()**
- **[dropwhile(predicate, iterable)](#dropwhile%20predicate%20iterable)** - итератор зна 
- **takewhile()**
- **filterfalse()**
- **groupby()**
...
Функция `dropwhile()` возвращает итератор, который генерирует элементы входного итерируемого объекта сразу же после того, как для заданного условия будет получено ложное значение. Содержимое, предшествующее ложному значению условия отбрасывается
