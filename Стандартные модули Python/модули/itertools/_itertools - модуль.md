...
|**[`zip_longest(*iterables, *, fillvalue=None)`](#`zip_longest(iterables,%20*,%20fillvalue=None)`)**|аналог `zip()` для объединения последовательностей разной длины|
...
|**[product(\*arrays)](#product%20arrays)**|итератор декартовых произведений (перебор комбинаций значений для каждой позиции)|
...
Элементы итерируемого объекта рассматриваются как уникальные в зависимости от их положения, а не от их значения. Поэтому, если элементы уникальны, повторных значений в каждом сочетании не будет. Если же не уникальны, то будут повторы
...
Если значение аргумента `r` не указано или равно `None`, тогда по умолчанию `r` равно длине итерируемого объекта `iterable`, и генерируются все возможные перестановки полной длины. Если `r` больше длины, вернется пустое значение
...
### `product(*arrays)` 
Предназначена для создания декартовых произведений из серии входных итераций (CROSS JOIN).
Простыми словами: итератор комбинаций путем перебора вариантов для каждой 
позиции
